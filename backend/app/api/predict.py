...
def _fetch_urls_text(urls: list) -> str:
    ...
    back to prompt-synthesis if nothing is recoverable. Per-URL results
    are cached for 1 day so retries / parallel runs don't re-fetch.
    """
    import hashlib
    ...
    sections = []
    for url in urls:
        cache_key = "url_text:" + hashlib.sha256(url.encode("utf-8")).hexdigest()[:24]
        ...
        try:
            ...
            cache_set(cache_key, section, ttl=60 * 60 * 24)
...
def _synthesize_seed_brief(simulation_requirement: str) -> str:
    ...
    LLM-generated so downstream steps can surface that provenance. Cached
    for 7 days keyed on the prompt so retries don't re-pay the cost.
    """
    import hashlib
    from ..utils.llm_client import LLMClient
    from ..services.cache_service import cache_get, cache_set

    cache_key = "seed_brief:" + hashlib.sha256(
        (simulation_requirement + "|" + (Config.LLM_CHEAP_MODEL_NAME or "")).encode("utf-8")
    ).hexdigest()[:24]
    cached = cache_get(cache_key)
    if isinstance(cached, str) and cached:
        logger.info("seed brief cache hit")
        return cached
    ...
    final = header + (body or simulation_requirement)
    cache_set(cache_key, final, ttl=60 * 60 * 24 * 7)
    return final

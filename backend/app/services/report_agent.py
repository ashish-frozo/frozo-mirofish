...
4. Support conversation with users, autonomously calling retrieval tools during chat
"""
...
[Returns]
- Relevant facts verbatim (can be directly cited)
...
This is not LLM simulation, but calling real interview endpoints to get original responses from simulated agents.
...
[Use Cases]
- Need different role perspectives on an event (How do students see it? How does media see it? What do officials say?)
...
PLAN_SYSTEM_PROMPT = """\
You are a "Future Prediction Report" writing expert with a "God's-eye view" of the simulated world — you can observe every agent's behavior, statements, and interactions in the simulation.

[Core Concept]
We have built a simulated world and injected a specific "simulation requirement" as a variable. The evolution results of the simulated world represent predictions of what could happen in the future. You are observing not "experimental data" but a "rehearsal of the future."
...
3. [Language Consistency - Quoted Content Must Match Report Language]
   ...
   - When quoting tool-returned content in a different language, translate it into the report's language before including it
...
Social media served as the primary channel for breaking news, carrying the core function of initial information dissemination:
...
Each response must do exactly one of the following (not both):
...
The system will execute the tool and return results to you. You must not and cannot write tool results yourself.
...
4. [Quote Format Rules - Must Be Separate Paragraphs]
   Quotes must stand alone as separate paragraphs, with a blank line before and after; do not embed them within a paragraph:
...
2. You must call tools to obtain simulation data before writing
...
REACT_UNUSED_TOOLS_HINT = "\n💡 You haven't used yet: {unused_list} — consider trying different tools for multi-perspective information"
...
class ReportAgent:
    ...
    def _generate_section_react(
        ...
        # Build user prompt - each completed section is passed in with a max of 4000 characters
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls:
                # Tool quota exhausted -> explicitly inform and require Final Answer output
            ...
            # Directly use this content as the final answer to avoid idle iterations
    ...
    def generate_report(
        ...
        try:
            ...
            # Stage 2: Generate sections one by one (save per section)
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Line number to start reading from (for incremental retrieval, 0 = from beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if this is a duplicate of the section title (skip duplicates within first 5 lines)
                if i < 5:

        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be started during a live run and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Stores the pending target state
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Clear previous debounce timer (new scroll intent overrides the old one)
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom upward so more scrolling is needed to trigger expansion
...
.grid-pattern {
  ...
  /* Position from top-left; height changes only extend at bottom without affecting existing grid positions */

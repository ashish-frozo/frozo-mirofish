              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When it is a final answer, show special indicator -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const relMatch = text.match(/Relations:\s*(\d+)/)
    ...
    // Extract Relations - full extraction, no limit
    const relSection = text.match(/### \[Relations\]\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract Related Entities - full extraction, no limit
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **Alumni_345 (index=1)**: As an alumnus...
        ...
        // e.g.: - Selected Parent_601 (index 0): As a parent representative...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:Selected|选择)([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        if (name) {
          ...
          // Continuation of reason (exclude concluding summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatibility: only one platform marker)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default display when not placeholder text
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest key facts from episodic memory'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Support multiple numbering formats:
...
// Current active step (shown at top)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 title (## xxx), as section title is displayed in outer wrapper
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled in the nextTick after loop ends
          }

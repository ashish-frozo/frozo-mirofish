  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Previous simulation logs cleared, restarting simulation')
      }
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Additional check: backend may not have updated runner_status yet,
      // but platforms already report completion
...
const checkPlatformsCompleted = (data) => {
  ...
  // A platform is considered enabled if count > 0 or was running or completed
